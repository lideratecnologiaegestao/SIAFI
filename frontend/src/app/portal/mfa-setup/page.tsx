...
export default function PortalMfaSetupPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.mfa.enroll({ factorType: 'totp', issuer: 'SIAFI', friendlyName: 'Google Authenticator' })
      .then(({ data, error: enrollError }: any) => {
        if (enrollError) throw enrollError
        setEnrollment(data as unknown as EnrollData)
        setLoading(false)
      })
      .catch((e: any) => { setError(e?.message ?? 'Erro ao iniciar configuração.'); setLoading(false) })
  }, [])
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      setDone(true)
      setTimeout(() => router.replace(getRedirectParam()), 2000)
    } catch (e: any) {
