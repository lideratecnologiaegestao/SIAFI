...
export default function LoginPage() {
  ...
  // Read OAuth error from query string without requiring useSearchParams + Suspense
  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    const err = params.get('error')
    if (err) setServerError(OAUTH_ERRORS[err] ?? 'Erro de autenticação.')
  }, [])
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      // loginWithGoogle redirects — execution stops here
    } catch {
      setServerError('Erro ao iniciar login com Google.')
